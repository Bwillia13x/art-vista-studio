...
import { format, isAfter, isSameDay, setHours, setMinutes, startOfDay, startOfToday } from "date-fns";
import {
  ArrowRight,
  BadgeCheck,
  CalendarDays,
...
type CustomerDetails = typeof INITIAL_CUSTOMER;
...
function formatTimeDisplay(date: Date, time: string) {
  const [hours, minutes] = time.split(":").map(Number);
  return format(setMinutes(setHours(date, hours), minutes), "h:mm a");
}

function stylistWorksOnDate(stylist: Stylist, date: Date) {
  return stylist.schedule.some((slot) => slot.day === date.getDay());
}

function getScheduleForDate(stylist: Stylist, date: Date): StylistScheduleEntry | undefined {
...
export default function Booking() {
  ...
  const availableSlots = useMemo(() => {
    ...
    const stylistBookings = bookings.filter(
      (booking) => booking.stylistId === selectedStylist.id && booking.date === format(selectedDate, "yyyy-MM-dd"),
    );

    return generateSlots(schedule, selectedDate, totalDuration, stylistBookings);
  }, [selectedService, selectedStylist, selectedDate, totalDuration, bookings]);
  ...
  const detailsValid = Boolean(customer.name && customer.email && customer.phone);
  ...
  const handleConfirmBooking = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const addOnSnapshot = [...addOnSelections];
    ...
    const confirmationCode = `BRG-${bookingId.slice(-6).toUpperCase()}`;
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: `${selectedService.name} on ${format(selectedDate, "EEEE, MMMM do")} at ${formatTimeDisplay(selectedDate, selectedTime)} with ${selectedStylist.name}.`,
      });
          ...
          return (
            <button
              ...
              className={`text-left rounded-xl border transition-all duration-300 ${
                isSelected ? "border-primary bg-primary/5 shadow-lg shadow-primary/20" : "border-primary/10 hover:border-primary/30"
              }`}
            >
  ...
  const renderDetailsStep = () => (
    <form onSubmit={handleConfirmBooking} className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="flex items-start gap-3 pt-6">
          <Checkbox
            ...
            onCheckedChange={(value) => setCustomer({ ...customer, marketing: Boolean(value) })}
          />
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between pt-4 border-t border-border/70">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:order-3">
          <p className="text-sm text-muted-foreground">
            You're booking {selectedService?.name} on {selectedDate ? format(selectedDate, "MMM d") : "--"} at {" "}
