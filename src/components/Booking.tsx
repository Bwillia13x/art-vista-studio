...
// Simplified booking type without database references for now
type BookingRowSimple = {
  id: string;
  service_id: string;
  stylist_id: string;
...
export default function Booking() {
  ...
  const stylistsQuery = useQuery({
    queryKey: ["stylists"],
    queryFn: async () => {
      // Fetch stylists only - specialties table doesn't exist yet
      ...
      // Return stylists with empty specialties array (all stylists available for all services)
      const enrichedStylists = stylistsData.map((stylist) => ({
        ...stylist,
        stylist_specialties: [], // Empty for now - all stylists can do all services
      }));

      return (enrichedStylists as any[]).map(mapStylist);
    },
  });
  ...
  const availableStylists = useMemo(() => {
    ...
    // If no specialties defined, show all stylists (until migrations are run)
    const filtered = stylists.filter((stylist) =>
      stylist.specialties.length === 0 || stylist.specialties.includes(selectedService.id)
    );
    return filtered.length > 0 ? filtered : stylists; // Fallback to all if filter returns empty
  ...
  const bookingsQuery = useQuery({
    ...
    staleTime: 1000 * 30,
    queryFn: async () => {
      // Return empty array for now - no bookings table yet
  ...
  const bookingMutation = useMutation({
    mutationFn: async (submission: {
      ...
      // For now, just return a mock booking
      const mockBooking: BookingRecord = {
        id: `booking-${Date.now()}`,
        serviceId: submission.service.id,
        stylistId: submission.stylist.id,
        date: submission.date,
        time: submission.time,
        duration: submission.duration,
        addOnIds: submission.addOnIds,
        clientName: submission.customer.name,
        clientEmail: submission.customer.email,
        clientPhone: submission.customer.phone,
        notes: submission.customer.notes || null,
        marketingConsent: submission.customer.marketing,
      };
      return {
        booking: mockBooking,
        submission,
          ...
          : services.map((service) => {
            ...
            return (
              <button
                ...
                className={`text-left rounded-xl border transition-all duration-500 ease-out ${isSelected
                  ? "border-primary bg-primary/5 shadow-lg shadow-primary/20 scale-[1.01]"
                  : "border-primary/10 hover:border-primary/30 hover:shadow-md hover:shadow-primary/10"
                  }`}
                style={{
            ...
            {recommendedAddOns.length === 0 ? (
              ...
              recommendedAddOns.map((addOn) => {
                ...
                return (
                  <label
                    key={addOn.id}
                    className={`group flex h-full cursor-pointer flex-col justify-between rounded-xl border p-4 transition-all duration-300 ease-out ${checked
  ...
  const renderScheduleStep = () => (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <Badge variant={selectedStylistId ? "default" : "outline"} className="gap-1.5">
          {selectedStylistId ? <Check className="h-3 w-3" /> : <span className="h-3 w-3 rounded-full border-2 border-current" />}
          <span>1. Select stylist</span>
        </Badge>
        <Badge variant={selectedDate ? "default" : "outline"} className="gap-1.5">
          {selectedDate ? <Check className="h-3 w-3" /> : <span className="h-3 w-3 rounded-full border-2 border-current" />}
          <span>2. Choose date</span>
        </Badge>
        <Badge variant={selectedTime ? "default" : "outline"} className="gap-1.5">
          {selectedTime ? <Check className="h-3 w-3" /> : <span className="h-3 w-3 rounded-full border-2 border-current" />}
          <span>3. Pick time</span>
        </Badge>
      </div>
          ...
          {!selectedStylistId && (
            <Alert className="border-primary/40 bg-primary/5">
              ...
              <AlertDescription className="text-sm">
                <span className="font-semibold text-primary">Step 1: Choose your stylist</span>
                <br />
                Select one of the stylists below to view their availability
              </AlertDescription>
            </Alert>
          )}
              ...
              : availableStylists.map((stylist) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl border p-5 text-left transition-all duration-300 hover:scale-[1.02] ${isSelected
              ...
              {!selectedStylistId ? (
                <div className="py-12 text-center space-y-4">
                  ...
                  <div className="space-y-2">
                    <p className="font-semibold text-foreground">Select a stylist first</p>
                    <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                      Choose one of the stylists on the left to see their available dates and times
                    </p>
                  </div>
                </div>
              ) : (
                      ...
                      <Alert className="mb-4 border-primary/40 bg-primary/5">
                        ...
                        <AlertDescription className="text-sm">
                          <span className="font-semibold text-primary">
                            {formatTimeDisplay(selectedDate!, selectedTime)}
                          </span>
                          {" selected. "}
                          <span className="hidden md:inline">Click "Continue" below to proceed.</span>
                          <span className="md:hidden">Tap "Continue" at the bottom to proceed.</span>
                        </AlertDescription>
                      </Alert>
                    )}
      ...
      {/* Sticky Continue Button for Mobile - Only show when ready to advance */}
      {canAdvanceFromSchedule && (
        <div className="md:hidden fixed bottom-0 left-0 right-0 p-4 bg-background/95 backdrop-blur-sm border-t border-primary/20 shadow-lg z-40 animate-in slide-in-from-bottom-4 duration-300">
          <Button
            type="button"
  ...
  const stepControls = (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-t border-border/60 pt-4">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        {currentStep === 1 && !canAdvanceFromSchedule && (
                  ...
                  {steps.map((step, index) => {
                    ...
                    return (
                      <div key={step} className="flex items-center gap-4">
                        <div
                          className={`flex h-10 w-10 items-center justify-center rounded-full border text-sm font-semibold transition-all ${isActive
